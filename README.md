...
Ollama запускается на хостовой системе, а приложение в контейнере обращается к ней через адрес `host.docker.internal`. Docker Compose автоматически передаёт этот адрес в Streamlit, поэтому в интерфейсе доступны обе локально установленные модели: `qwen2.5:3b` и `mistral:7b`. Для работы генерации Ollama должна быть запущена и принимать подключения от Docker; если она недоступна, поиск и режим `extractive` продолжают работать независимо от LLM. Кэш embedding-модели сохраняется в `.codelens/huggingface` и переиспользуется после пересоздания контейнера. Ollama также сохраняет скачанные модели на хосте и некоторое время удерживает использованную модель в памяти, ускоряя следующие ответы.
